...
router.post('/delete/:_id', (req, res) => {
   // postdb.findOneAndDelete({_id: req.params.id}
    ...
    postdb.deleteOne({_id: req.body.deleteID}, function(err){
        if(err){
            console.log(err);
        }
    console.log(req.body.deleteID + req.params.id );
...
router.get('/confirmBox', (req, res) => {
    res.render('confirmBox')
});
...
/*router.get('/youreporter', (req, res) => {
    res.render('youreporter')
});*/

/*router.get('/youreporter', (req, res) => {
    tvProgram.find({ category: { $ne: null } }, function(err, category, next) {
    if (err) {
    console.log(err + "the error");
    } else {
    //res.json('posts', {category:docs});
    // res.send(docs);
    res.render('youreporter', { category: category });
    }
    }).sort({ category: 1 });
